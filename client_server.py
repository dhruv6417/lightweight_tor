...
def generate_circuit(nodes):
    ...
    """
    circuit = [(str(ip), crypty.gen_aes_key()) for ip in nodes.keys()]
    shuffle(circuit)
    return circuit
...
def encrypt_payload(message, circuit, relay_nodes):
    ...
    while len(node_stack) != 0:
        ...
        # encrypt payload
        payload = crypty.encrypt(curr_aes_key_instance, public_key, (payload + next.encode()))
        next = curr_node_addr
...
def decrypt_payload(payload, circuit):
    ...
    for i in range(len(circuit)):
        ...
        message = crypty.decrypt_aes(aes_key, decoded_message)
